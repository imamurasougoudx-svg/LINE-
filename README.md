...
const API_BASE_URL = import.meta.env.VITE_API_BASE_URL || "https://example-hospital-api.local";
...
async function fetchPatientProfile(idToken) {
  if (ENABLE_MOCK) {
...
function LoadingScreen({ text = "患者ポータルの利用準備を進めています。" }) {
  return (
...
function ErrorScreen({ errorMessage, onRetry, onUseMock }) {
  return (
    <div className="flex min-h-screen items-center justify-center bg-slate-100 p-4">
      <div className="w-full max-w-md rounded-[28px] bg-white p-8 shadow-xl ring-1 ring-slate-200">
        ...
        <div className="mt-4 grid gap-2">
          <button onClick={onRetry} className="rounded-2xl bg-slate-900 px-4 py-3 text-sm font-semibold text-white">
            再試行
          </button>
          <button onClick={onUseMock} className="rounded-2xl bg-slate-100 px-4 py-3 text-sm font-semibold text-slate-700">
            モックで起動
          </button>
        </div>
      </div>
...
function LoginRequiredScreen({ onLogin, isInClient }) {
  return (
    <div className="flex min-h-screen items-center justify-center bg-slate-100 p-4">
      <div className="w-full max-w-md rounded-[28px] bg-white p-8 shadow-xl ring-1 ring-slate-200">
        ...
        <div className="mt-4 space-y-3 rounded-3xl bg-slate-50 p-4 ring-1 ring-slate-200">
          <div className="rounded-2xl bg-white p-3 ring-1 ring-slate-200">
            ...
            <p className="mt-1 text-xs text-slate-500">1. LINEログイン → 2. 患者確認 → 3. あと払い設定</p>
          </div>
          <button onClick={onLogin} className="w-full rounded-2xl bg-emerald-600 px-4 py-3 text-sm font-semibold text-white">
            LINEでログイン
          </button>
...
function AppShell({ children, activeTab, onTabChange, title, showBack, onBack, patient }) {
  return (
        ...
        <header className="bg-gradient-to-b from-emerald-500 to-emerald-600 px-5 pb-5 pt-6 text-white">
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              {showBack ? (
                <button onClick={onBack} className="rounded-2xl bg-white/15 p-2 transition hover:bg-white/20" aria-label="戻る">
...
function StatusRow({ label, value, emphasis = false }) {
  return (
    <div className="flex items-center justify-between rounded-2xl bg-white px-3 py-2 ring-1 ring-slate-200">
      <span>{label}</span>
      <span className={emphasis ? "font-semibold text-emerald-700" : "font-medium text-slate-700"}>{value}</span>
    </div>
  );
}

function LiffDebugPanel({ liffState }) {
  return (
    <section className="rounded-3xl bg-slate-50 p-4 ring-1 ring-slate-200">
      ...
      <div className="mt-3 space-y-2 text-sm text-slate-600">
        ...
        <StatusRow label="LINEログイン" value={liffState.isLoggedIn ? "ログイン済み" : "未ログイン"} emphasis={liffState.isLoggedIn} />
        <StatusRow label="LINE内起動" value={liffState.isInClient ? "はい" : "いいえ"} emphasis={liffState.isInClient} />
...
function PrimaryHero({ onStart, patient }) {
  return (
    <section className="rounded-3xl bg-gradient-to-b from-emerald-500 to-emerald-600 p-4 text-white shadow-sm">
      ...
      <div className="mt-2 flex items-end justify-between gap-3">
        ...
        <button onClick={onStart} className="rounded-2xl bg-white px-4 py-3 text-sm font-semibold text-emerald-700 shadow-sm">
          利用開始
        </button>
      </div>
    </section>
...
function SummaryCard({ title, value, sub, action, icon: Icon, onClick }) {
  return (
...
function MenuGrid({ onOpenHistory, onOpenFamily }) {
  return (
    <section>
...
function HomeScreen({ navigate, patient, liffState }) {
  ...
  return (
    <section className="space-y-4">
      <div className="rounded-3xl bg-slate-900 p-4 text-white shadow-sm">
        ...
        <button className="mt-4 rounded-2xl bg-white px-4 py-3 text-sm font-semibold text-slate-900">あと払い設定を確認</button>
      </div>
      <div className="space-y-3">
        {payments.map((item) => (
          <div key={item.id} className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div className="mt-4 grid grid-cols-2 gap-2">
              ...
              <button className="rounded-2xl bg-slate-100 px-4 py-3 text-sm font-medium text-slate-700">領収書確認</button>
            </div>
          </div>
        ))}
      </div>
...
function MyPageScreen({ patient, profile }) {
  return (
    <section className="space-y-4">
      <div className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="flex items-center justify-between gap-3">
        ...
        <button className="mt-4 w-full rounded-2xl bg-slate-900 px-4 py-3 text-sm font-medium text-white">診察券QRを表示</button>
      </div>
...
export default function PatientLinePostpayPortalLiffClean() {
  ...
  const bootApp = async () => {
    ...
    try {
      if (ENABLE_MOCK || LIFF_ID === "YOUR_LIFF_ID") {
      ...
      setPatient({
        ...patientData,
        name: patientData.name || liffProfile.displayName,
        lineStatus: "LINEログイン済み",
      });
      setLiffState({
        isReady: true,
  ...
  if (bootState === "error") {
    return <ErrorScreen errorMessage={errorMessage} onRetry={bootApp} onUseMock={handleUseMock} />;
  }
